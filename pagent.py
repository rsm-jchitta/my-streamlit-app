...
def send_email(to_email, subject, content):
    ...
    SENDGRID_API_KEY ='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'
    ...
    if username:
        ...
        if folder_exists(S3_BUCKET_NAME, user_folder):
            ...
            if training_list:
                ...
                st.warning("⚠ No trainings found for this user.")
        else:
...
if "username" in locals() and folder_exists(S3_BUCKET_NAME, f"{username}/"):
    ...
    if st.session_state["new_training"]:
        ...
        if st.button("Cancel New Training", key="cancel_new_training"):
            st.session_state["new_training"] = False  # ✅ Reset training state
            st.rerun()  # ✅ Refresh UI to show old options again
          ...
          # Replace with your Replicate username
            ...
            if person1 and person2:
                ...
                if st.button("Start Training"):
                    st.session_state["new_training"] = False  # ✅ Reset to hide training form after submission
                    st.rerun()  # ✅ Refresh UI

                    if len(person1_images) == 1 and len(person2_images) == 1 and len(combined_images) == 1:
                                    ...
                                    messages=[
                                        {
                                            "role": "user",
                                            "content": [
                                                {"type": "text", "text": f"Describe this image in one sentence. Always start your sentence with 'A photo of {folder.replace('_', ' and ')}' even if you cant say anything."},
