                    ...
                    if len(person1_images) >= 5 and len(person2_images) >= 5 and len(combined_images) >= 5:
                        for image, folder in zip(
                            ...
                            for img in image:
                                ...
                                # ✅ Store captions for editing before saving
                                edited_captions = {}
                                ...
                                for image, folder in zip(
                                    ...
                                    for img in image:
                                        ...
                                        # ✅ Show caption beside the image name for user editing
                                        edited_caption = st.text_area(f"Edit caption for {img.name}:", value=generated_caption, key=img.name)

                                        # ✅ Store the edited caption for later submission
                                        edited_captions[img.name] = edited_caption

                                # ✅ Add a submission button to save edited captions to S3
                                if st.button("Save Captions"):
                                    for img_name, final_caption in edited_captions.items():
                                        ...
                                        # ✅ Upload the edited caption as a text file to S3
                                        s3_client.put_object(
                                            Bucket=S3_BUCKET_NAME,
                                            Key=txt_file_key,
                                    ...
                                    st.success("✅ Edited captions saved successfully!")
