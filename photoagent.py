            ...
            if status == "succeeded":
                ...
                if training.output and "version" in training.output:
                    ...
                    try:
                        ...
                        s3_client.put_object(
                            Bucket=S3_BUCKET_NAME,
                            Key=f"{training_folder}trigger_word.txt",
                            Body=TRIGGER_WORD.encode("utf-8"),
                            ContentType="text/plain",
                        )
                ...
                if selected_training:
                    ...
                    if not st.session_state.get("new_training", False):  
                        try:
                            ...
                            model_content = model_obj["Body"].read().decode("utf-8").strip()  # ✅ Strip whitespace

                            if model_content:  # ✅ Ensure it's not empty
                                trained_model_id = model_content
                                st.success(f"✅ Model Loaded: {trained_model_id}")
                            else:
                                st.error("❌ Model file exists but is empty! Training might not be complete.")
                                trained_model_id = None  # ✅ Prevent using empty model ID
                    ...
                    # ✅ Prompt input and image generation
                    prompt = st.text_input("Enter a prompt to generate an image(e.g. A man(please dont give persons name) and woman sitting on a bench)")
                    ...
                    if st.session_state.get("generate_image", False):
                        ...
                        if prompt:
                            # ✅ Load the trigger word from S3
                            trigger_word_key = f"{user_folder}{selected_training}/trigger_word.txt"

                            try:
                                obj = s3_client.get_object(Bucket=S3_BUCKET_NAME, Key=trigger_word_key)
                                TRIGGER_WORD = obj["Body"].read().decode("utf-8")
                            except Exception as e:
                                st.warning("⚠ Trigger word not found. Using default behavior.")
                                TRIGGER_WORD = ""  # Default to empty if not found

                            # ✅ Modify the user’s prompt to include the trigger word
                            modified_prompt = replace_names_with_codewords(prompt, name_codeword_dict)
                            final_prompt = f"{TRIGGER_WORD} {modified_prompt}"  # ✅ Add trigger word before prompt
...
if "username" in locals() and folder_exists(S3_BUCKET_NAME, f"{username}/"):
    ...
    if st.session_state["new_training"]:
        ...
        training_name = st.text_input("Enter training name(recommended format: Person1 and Person2)")
        ...
        if training_name:
          ...
          # Replace with your Replicate username
            ...
            # ✅ Generate a 4-letter trigger word for this model
            import random
            import string

            # ✅ Generate a 4-letter uppercase alphabet trigger word
            def generate_trigger_word(model_name):
                """Generates a unique 4-letter trigger word using only uppercase letters."""
                random.seed(hash(model_name))  # Use model name hash as seed for consistency
                return ''.join(random.choices(string.ascii_uppercase, k=5))  # Pick 4 uppercase letters

            # ✅ Generate trigger word for this model
            TRIGGER_WORD = generate_trigger_word(MODEL_NAME)

            # ✅ Debugging Print Statement
            st.write(f"🔍 Generated Trigger Word: {TRIGGER_WORD}")  
            # Convert to lowercase

            TRIGGER_WORD = generate_trigger_word(MODEL_NAME)  # ✅ Generate trigger word
                ...
                if st.button("Start Training"):
                    if not st.session_state.get("training_started", False):
                        if len(person1_images) < 5 or len(person2_images) < 5 or len(combined_images) < 5:
                            ...
                            st.session_state["training_started"] = True  # ✅ Set training state only if images are uploaded
                                    ...
                                    messages=[
                                        {
                                            "role": "user",
                                            "content": [
                                                {"type": "text", "text": "Describe this image in one sentence(atleast 10 words mandatory)."},
